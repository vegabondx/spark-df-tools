...
object Functions {
  ...
  def seqToCol(colString: String, char: Char = ','): Seq[Column] = colString.split(char).map(c => col(c))
  ...
  def getColValueType(clmn: Column): Column =
    when(clmn.isNull, "isNull")
      ...
      .when(clmn === "", "isBlank")
      .when(lower(clmn) === "null" || lower(clmn) === "nan" || lower(clmn) === "none" || lower(clmn) === "na" || lower(clmn) === "n/a", "isNullString")
  ...
  def getColValueTypeStats(df:DataFrame,clmns: Seq[String]):DataFrame =
    clmns.foldLeft(df)((d,c)=>d.withColumn(c+"_valType",getColValueType(col(c))))
}